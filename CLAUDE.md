...
| Payments | Stripe Connect Express onboarding via authenticated **`/api/stripe/connect/start`** + **`/api/stripe/connect/status`**; repeat connect reuses the saved connected account, first-time prefill stays minimal to preserve Stripe’s reuse path, invoice payment links + **`POST /api/stripe/webhook`** |
...
- **Stripe Connect behavior:** IronWork stays on **Express**. A profile with an existing **`stripe_account_id`** always reuses that same connected account when the user clicks **Connect Stripe** again; the server reconciles Stripe status before issuing a fresh onboarding link. First-time account creation only prefills **email**, **business name**, optional absolute-HTTPS **business_profile.url**, and the required MCC so Stripe-hosted/networked onboarding can still offer existing-account reuse when eligible. Do not prefill country, address, identity/person, phone, or bank fields from Edit Profile into Stripe.
...
- Invoice business state: no invoice row shows **Invoice**, an existing invoice with `issued_at = null` shows **Draft**, `issued_at != null` shows **Invoiced** (payment-link creation sets `issued_at`), and `payment_status = 'paid'` shows **Paid**; `payment_status = 'offline'` shows **Paid Offline** on `WorkOrdersPage` (page-scoped row buttons), `InvoiceFinalPage`, and `WorkOrderDetailPage` (Stripe webhook updates Postgres; **`InvoiceFinalPage`** refetches the invoice once on mount).
- Profile-completion nudge on **Work Orders** (missing business phone): **Not now** hides the full nudge; dismissal is stored in **`localStorage`** per `userId` with a timestamp and expires after **48 hours** (visibility recomputes on `userId` change).
